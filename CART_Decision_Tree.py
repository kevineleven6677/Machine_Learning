...
'''For each column j find its gini score'''
output={}
for j in range(col_size):
    gini=[]
    vector=unique(getVector(training_x,j))
    ...
    for k in range(len(vector)):
        ...
        for i in range(training_size):
            if  training_x[i][j] <= vector[k]:
                lsize += 1
                if training_r[i] == -1:
                    ln += 1
            elif  training_x[i][j] > vector[k]:
                rsize +=1
                if training_r[i] == -1:
                    rn += 1
        if lsize == 0:
            ...
            gini.append(lsize/training_size*ln/lsize*(1-ln/lsize)+
                        rsize/training_size*rn/rsize*(1-rn/rsize))
    #output[j]=vector[gini.index(min(gini))]
    vector_split=vector[gini.index(min(gini))]
    print(j,vector_split)
